...
def calculateAveragePrices(dataframe, csv = False, readableTime = False):
...
#       
    ...
    if readableTime == True: 
        for binanceDate in CompleteDataFrame['time']: 
        ...
        for i in range(len(df["time"])):
            if i+1>len(df["time"]):
                ...
                if (i+1 < len(df["time"]) and df[:,"time"][i+1]-df[:,"time"][i]>0):
                    print("true")
                else: 
                    print("false")
        CompleteDataFrame['time'] = pd.Series(Dates)
    if csv == True: 
...
calculateAveragePrices(tradeHistorydf, csv = True, readableTime=False)
